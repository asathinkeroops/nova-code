...
export const zh: DeepPartial<Catalog> = {
  ...
  effort: {
    ...
    blurbAuto: "使用 provider 或模型默认值，不显式发送推理设置。",
    blurbOff: "关闭扩展思考 —— 回复最快。适合简单编辑和快速提问。",
    blurbLow: "轻度推理，适合直接的单步任务。",
    blurbMedium: "均衡推理，适合日常工作。",
    blurbHigh: "深度推理，适合值得额外延迟的多步难题。",
    blurbMax: "最强推理 —— 仅在最难的任务上慎用。",
    setTo: "思考等级已设为",
    expected: "预期为 auto|off|low|medium|high|max",
    saveFailed: (msg: string): string => `保存设置失败：${msg}`,
  },
  ...
  doctor: {
    ...
    unknownProviderDetail:
      '使用 "generic" 通用 profile 回退（提供标准思考等级映射，无供应商专属错误翻译或余额查询）',
    unknownProviderHint: (providers: string): string =>
      `将当前 providers[] 连接的 "profile" 设为以下之一：${providers} —— 普通兼容端点也可保持不变`,
  ...
  setup: {
    tagline: "面向国产大模型的编程智能体 —— 高缓存命中率 · 系统级沙箱 · 工具齐备",
    welcome: "欢迎使用 Nova！",
    ...
    manualIntro: (path: string): string => `要使用自定义服务商，请在此创建配置文件：${path}`,
    manualShape: "配置结构如下：",
  ...
  ask: {
    ...
    navOption: "↑/↓ 选择",
    navButton: "↑/↓ 按钮",
    navPick: (count: number): string => (count > 1 ? `1-${count} 直选` : "1 直选"),
    navToggle: "空格 切换",
    navQuestion: "←/→ 切换标签",
